...
void CAN_Init(const Can_ConfigType *pConfig)
{
    ...
    uint32_t ctrl1Temp;
    uint32_t instance;
    instance = FLEXCAN_GetInstance(CAN0);
    ...
    FLEXCAN_SetBitRate(CAN0, EXAMPLE_CAN_CLK_FREQ, pConfig->bitRate, pConfig->timingConfig);
}
/*!
 ...
 */
Std_ReturnType Can_WriteTxMb(uint8_t mbIdx, const flexcan_rx_mb_config_t *pTxFrame)
{
    /* Assertion. */
	/*
    assert(mbIdx <= (base->MCR & CAN_MCR_MAXMB_MASK));
    assert(NULL != pTxFrame->kFrame);
    assert(pTxFrame->kFrame.length <= 8U);
    assert(!FLEXCAN_IsMbOccupied(base, mbIdx));*/

    uint32_t cs_temp = 0;
    Std_ReturnType status;

    FLEXCAN_SetRxMbConfig(EXAMPLE_CAN_, RX_MESSAGE_BUFFER_NUM_, &pTxFrame, true);
    /* Setup Tx Message Buffer. */
    FLEXCAN_SetTxMbConfig(EXAMPLE_CAN_, TX_MESSAGE_BUFFER_NUM_, true);
    /* Check if Message Buffer is available. */
    if (CAN_CS_CODE(kFLEXCAN_TxMbDataOrRemote) != (EXAMPLE_CAN_->MB[mbIdx].CS & CAN_CS_CODE_MASK))
    {
        /* Inactive Tx Message Buffer. */
    	EXAMPLE_CAN_->MB[mbIdx].CS = (EXAMPLE_CAN_->MB[mbIdx].CS & ~CAN_CS_CODE_MASK) | CAN_CS_CODE(kFLEXCAN_TxMbInactive);

        /* Fill Message ID field. */
    	EXAMPLE_CAN_->MB[mbIdx].ID = pTxFrame->kFrame.id;

        /* Fill Message Format field. */
        if ((uint32_t)kFLEXCAN_FrameFormatExtend == pTxFrame->kFrame.format)
        {
            cs_temp |= CAN_CS_SRR_MASK | CAN_CS_IDE_MASK;
        }

        /* Fill Message Type field. */
        if ((uint32_t)kFLEXCAN_FrameTypeRemote == pTxFrame->kFrame.type)
        {
            cs_temp |= CAN_CS_RTR_MASK;
        }

        cs_temp |= CAN_CS_CODE(kFLEXCAN_TxMbDataOrRemote) | CAN_CS_DLC(pTxFrame->kFrame.length);

        /* Load Message Payload. */
        EXAMPLE_CAN_->MB[mbIdx].WORD0 = pTxFrame->kFrame.dataWord0;
        EXAMPLE_CAN_->MB[mbIdx].WORD1 = pTxFrame->kFrame.dataWord1;

        /* Activate Tx Message Buffer. */
        EXAMPLE_CAN_->MB[mbIdx].CS = cs_temp;
...
/* Wait until CAN Message send out. */
        uint32_t u32flag = 1;
        while (0U == FLEXCAN_GetMbStatusFlags(EXAMPLE_CAN_, u32flag << TX_MESSAGE_BUFFER_NUM_))
        {
        }

        FLEXCAN_ClearMbStatusFlags(EXAMPLE_CAN_, u32flag << TX_MESSAGE_BUFFER_NUM_);
}
...
status_t FLEXCAN_TransferSendNonBlocking(CAN_Type *base, flexcan_handle_t *handle, flexcan_mb_transfer_t *pMbXfer)
{
    ...
    if ((uint8_t)kFLEXCAN_StateIdle == handle->mbState[pMbXfer->mbIdx])
    {
        ...
        if (kStatus_Success ==
            Can_WriteTxMb(pMbXfer->mbIdx, (const flexcan_frame_t *)(uint32_t)pMbXfer->frame))
        {
